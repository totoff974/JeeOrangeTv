#!/bin/bash
cd $1
touch /tmp/JeeOrangeTv_dep
echo "Début de l'installation"

echo 0 > /tmp/JeeOrangeTv_dep
DIRECTORY="/var/www"
if [ ! -d "$DIRECTORY" ]; then
...
echo 85 > /tmp/JeeOrangeTv_dep
sudo npm install --unsafe-perm request
echo 90 > /tmp/JeeOrangeTv_dep

sudo chown -R www-data *

rm /tmp/JeeOrangeTv_dep
