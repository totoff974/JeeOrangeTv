...
$("#table_liste_chaine").sortable({axis: "y", cursor: "move", items: ".cmd", placeholder: "ui-state-highlight", tolerance: "intersect", forcePlaceholderSize: true});
 ...
 function addCmdToTableChaine(_cmd) {
    ...
    if (init(_cmd.type) == 'info') {
        ...
        tr += '</td>';
        tr += '<td>';
        tr += '<input class="cmdAttr form-control input-sm" data-l1key="ch_epg" placeholder="{{EPG}}">';
        tr += '</td>';
        tr += '<td>';
        tr += '<input class="cmdAttr form-control input-sm" data-l1key="ch_logo" placeholder="{{Logo}}">';
        tr += '</td>';
        tr += '<td>';
