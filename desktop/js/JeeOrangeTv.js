...
$("#table_liste_chaine").sortable({axis: "y", cursor: "move", items: ".cmd", placeholder: "ui-state-highlight", tolerance: "intersect", forcePlaceholderSize: true});
 ...
 function addCmdToTableChaine(_cmd) {
    ...
    if (init(_cmd.type) == 'action') {
        ...
        tr += '</td>';
        tr += '<td>';
        tr += '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="ch_canal" type="range" min="0" placeholder="{{Canal}}">';
        tr += '</td>';
        tr += '<td>';
        tr += '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="ch_epg" type="range" min="-1" max="9999999999" placeholder="{{EPG}}">';
        tr += '</td>';
        tr += '<td>';
        tr += '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="ch_logo" placeholder="{{Logo}}">';
        tr += '</td>';
        tr += '<td>';
        tr += '<input class="cmdAttr form-control input-sm" data-l1key="configuration" data-l2key="ch_categorie" list="categorie_chaine">';
        tr += '<datalist id="caterogie_chaine">';
        tr += '  <option value="Généraliste">';
        tr += '  <option value="Information">';
        tr += '  <option value="Découverte et Art de vivre">';
        tr += '  <option value="Sports">';
        tr += '  <option value="Jeunes adultes">';
        tr += '  <option value="Jeunesse">';
        tr += '  <option value="Divertissement">';
        tr += '  <option value="Société et Culture">';
        tr += '  <option value="Musique">';
        tr += '</datalist>';
        tr += '</td>';
        tr += '<td>';
